...
| `EMAIL_ADDRESS`                     | The mailbox you authenticate to SMTP with — full email address (e.g. `info@otownmakerspace.dk`).                    | SMTP username *and* the default `From:` header on outbound mail. Deploy writes it to `/opt/secrets/app/email_address` (bind-mounted into the container at `/run/secrets/email_address`). Pairs with `EMAIL_PASSWORD`. |
